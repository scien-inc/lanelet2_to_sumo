...
def _write_tls_linksignal_params(
    net_path: str | Path,
    signal_mapping_path: str | Path,
    opendrive_mapping_path: str | Path,
) -> dict[str, object]:
    """Add synchronization metadata using the mapping of this final network.

    Keep OpenDRIVE IDs verbatim (including map-specific numeric prefixes).
    Invalid or insufficient mappings are reported without modifying the net.
    """
    mapping = json.loads(Path(opendrive_mapping_path).read_text(encoding="utf-8"))
    signal_ids = mapping["traffic_light_signal_mapping"]["lanelet2_tl_id_to_signal_ids"]
    document = json.loads(Path(signal_mapping_path).read_text(encoding="utf-8"))
    records = [record for record in document["sumo_link_to_lanelet_signal"] if record.get("sync_eligible")]
    tree = ET.parse(net_path)
    root = tree.getroot()
    logics = {logic.attrib["id"]: logic for logic in root.findall("tlLogic")}
    valid_links = {(c.get("tl"), _connection_link_index(c)) for c in root.findall("connection") if c.get("tl")}
    params: dict[tuple[str, int], set[str]] = defaultdict(set)
    skipped = []
    mapped_count = 0
    invalid_count = 0
    for record in records:
        tls_id, index = record.get("actual_sumo_tls_id"), record.get("linkIndex")
        logic = logics.get(tls_id)
        states = [phase.get("state", "") for phase in logic.findall("phase")] if logic is not None else []
        if type(index) is not int or (tls_id, index) not in valid_links or not states or not all(0 <= index < len(state) for state in states):
            invalid_count += 1
            skipped.append({"tls_id": tls_id, "linkIndex": index, "reason": "invalid_link_index"})
            continue
        eligible_ways = _record_string_list(record, "sync_lanelet_traffic_light_way_ids")
        identifiers = list(eligible_ways)
        # A regulatory-element mapping describes all its signal heads. Do not
        # reintroduce heads excluded by the existing timing-conflict check.
        if set(eligible_ways) == set(_record_string_list(record, "lanelet_traffic_light_way_ids")):
            identifiers += _record_string_list(record, "lanelet_regulatory_element_ids")
        tokens = set()
        for identifier in identifiers:
            values = signal_ids.get(identifier, [])
            if not isinstance(values, list):
                raise ValueError(f"Expected a list of OpenDRIVE signal IDs for {identifier}")
            for value in values:
                if isinstance(value, bool) or not isinstance(value, (str, int)):
                    raise ValueError(f"Invalid OpenDRIVE signal ID: {value!r}")
                value = str(value).removeprefix("od:")
                if not value or any(char.isspace() for char in value):
                    raise ValueError(f"Invalid OpenDRIVE signal ID: {value!r}")
                tokens.add("od:" + value)
        if not tokens:
            skipped.append({"tls_id": tls_id, "linkIndex": index, "reason": "signal_id_not_found"})
            continue
        params[tls_id, index].update(tokens)
        mapped_count += 1
    bindings: dict[str, list[tuple[str, int]]] = defaultdict(list)
    for key, tokens in sorted(params.items()):
        for token in sorted(tokens):
            bindings[token].append(key)
    conflicts = {}
    for token, links in bindings.items():
        tls_ids = {tls for tls, _ in links}
        if len(tls_ids) > 1 or any(
            len({_tls_state_category(phase.get("state", "")[index]) for _, index in links}) > 1
            for phase in logics[links[0][0]].findall("phase")
        ):
            conflicts[token] = [{"tls_id": tls, "linkIndex": index} for tls, index in links]
    coverage = mapped_count / len(records) if records else 0.0
    errors = []
    if invalid_count:
        errors.append("invalid_link_index")
    if not params:
        errors.append("no_signal_params")
    if coverage < 0.90:
        errors.append("coverage_below_0.90")
    if not errors:
        for tls_id, logic in logics.items():
            for param in list(logic.findall("param")):
                if param.get("key", "").startswith("linkSignalID:"):
                    logic.remove(param)
            for (target, index), tokens in sorted(params.items()):
                if target == tls_id:
                    ET.SubElement(logic, "param", {"key": f"linkSignalID:{index}", "value": " ".join(sorted(tokens))})
        ET.indent(tree, space="    ")
        tree.write(net_path, encoding="utf-8", xml_declaration=True)
    return {
        "source": str(opendrive_mapping_path),
        "eligible_record_count": len(records),
        "mapped_record_count": mapped_count,
        "coverage": coverage,
        "params_written": len(params) if not errors else 0,
        "unique_signal_count": len(bindings),
        "invalid_link_index_count": invalid_count,
        "skipped_records": skipped,
        "ambiguous_signal_bindings": conflicts,
        "errors": errors,
    }
